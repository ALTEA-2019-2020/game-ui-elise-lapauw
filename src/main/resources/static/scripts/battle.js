...
const battleApiUrl = "https://battle-api-lapauw.herokuapp.com";
...
async function startBattle(a, b){
    ...
    trainerCurrentPokemon = battle.trainer.battlePokemonList[0];
    opponentCurrentPokemon = battle.opponent.battlePokemonList[0];
...
async function pokemonEntersBattle(trainerName, pokemon, frontView){
    await showMessage(`${trainerName}'s ${pokemon.pokemonType.name} enters battle !`);

    const pokemonSprite = frontView ? pokemon.pokemonType.sprites.front_default : pokemon.pokemonType.sprites.back_default;
    const pokemonImageSelector = `[id='${trainerName}-pokemon-img']`;
    $(pokemonImageSelector).attr("src", pokemonSprite);
    $(`#${trainerName}-pokemon-name`).text(pokemon.pokemonType.name);
...
function updatePokemonView(trainerName, pokemon){
    ...
    pokemonHpBar.text(pokemon.currentHpLevel);
    pokemonHpBar.attr("aria-valuemax",pokemon.maxHp);
    pokemonHpBar.attr("aria-valuenow",pokemon.currentHpLevel);
    pokemonHpBar.css("width",`${pokemon.currentHpLevel * 100 / pokemon.maxHp}%` );
}

async function pokemonExitsBattle(trainerName, pokemon){
    await showMessage(`${trainerName}'s ${pokemon.pokemonType.name} exits battle !`);
...
function checkBattleEnd(){
    ...
    if(battle.trainer.battlePokemonList.every(poke => poke.ko === true)){
        // show the battle end
        showMessage(`${trainerName} lost the battle !`);
        // exitPokemon(trainerName, trainerCurrentPokemon);
        return true;
    }
    if(battle.opponent.battlePokemonList.every(poke => poke.ko === true)){
...
async function refreshBattle(){
    ...
    if(trainerCurrentPokemon.ko){
        await showMessage(`${trainerName}'s ${trainerCurrentPokemon.pokemonType.name} is KO !`);
        await pokemonExitsBattle(trainerName, trainerCurrentPokemon);

        const firstAliveTrainerPokemon = battle.trainer.battlePokemonList.find(poke => poke.ko !== true);
        trainerCurrentPokemon = firstAliveTrainerPokemon;
        await pokemonEntersBattle(trainerName, trainerCurrentPokemon, false);
    }

    if(opponentCurrentPokemon.ko){
        await showMessage(`${opponentName}'s ${opponentCurrentPokemon.pokemonType.name} is KO !`);
        await pokemonExitsBattle(opponentName, opponentCurrentPokemon);

        const firstAliveOpponentPokemon = battle.opponent.battlePokemonList.find(poke => poke.ko !== true);
...
async function iaTurn(){
    await showMessage(`${opponentCurrentPokemon.pokemonType.name} attacks !`);
...
async function playerCommand(command){
    disableControls();
    if("ATTACK" === command){
        await showMessage(`${trainerCurrentPokemon.pokemonType.name} attacks !`);
...
function updateBattleData(data){
    battle = data;
    trainerCurrentPokemon = battle.trainer.battlePokemonList.find(poke => poke.id === trainerCurrentPokemon.id);
    opponentCurrentPokemon = battle.opponent.battlePokemonList.find(poke => poke.id === opponentCurrentPokemon.id);
}
